...
### 📐 החלטות עיצוב ואינטגרציה (שלב ג')

במהלך מיזוג בסיסי הנתונים ואיחוד האגפים, התקבלו מספר החלטות עיצוב מבניות (Schema Changes) במטרה למנוע כפילויות, לנרמל את הנתונים ולשמור על שלמות המידע (Data Integrity):

*   **איחוד ישות החנויות ונירמול הכתובות (`STORE` & `LOCATION`):**
    *   הטבלה `STORE` הורחבה ונוספו אליה השדות הבאים מהאגף הלוגיסטי: `websiteurl`, `address`, ו-`region`.
    *   בוצע מיזוג נתונים מלא מטבלת הכתובות המקורית שלנו (`LOCATION`) ישירות לתוך שדות הכתובת החדשים בטבלת החנויות.
    *   **תוצאה:** טבלת `LOCATION` נמחקה לחלוטין כדי למנוע כפילויות ומבנה מסורבל.

*   **התאמת ישות המוצרים (`PRODUCT`):**
    *   התכונה `Kashrut` הוסרה מטבלת המוצרים הראשית והועברה לטבלת קשר ייעודית (`PRODUCT_KASHRUT`) כדי לאפשר תמיכה במספר סוגי כשרות למוצר במידת הצורך.
    *   נוספה התכונה `dateofmanufacture` (תאריך ייצור) ומוזג פנימה כל מידע המוצרים שקיבלנו מהאגף של שרה.

*   **מיזוג וסנכרון נתוני המלאי (`INVENTORY`):**
    *   ביצענו איחוד בין טבלת המלאי המקורית לטבלת המלאי החדשה שקיבלנו.
    *   **פתרון אתגר המפתחות:** מכיוון שבטבלה החדשה של שרה לא היה קיים שדה המפתח הזר `StoreID` (מלאי כללי), הוחלט במסגרת ה-Migration להעניק לכל שורות המלאי החדשות את ערך ברירת המחדל `1` בשדה `StoreID`, ובכך לשייך אותן לסניף הראשי של הרשת.

*   **קליטת ישויות לוגיסטיקה חדשות:**
    *   נוצרו כל הטבלאות הלוגיסטיות החדשות שלא היו קיימות באגף המקורי שלנו (כגון: `WAREHOUSE`, `DELIVERYCOMPANY`, `TRUCK`, `"ORDER"`, `CONTAINS`, `LOCATED` ועוד).
    *   כל המידע והנתונים מהגיבוי של שרה הועברו בהצלחה ובאופן מלא לתוך טבלאות אלו.

### כל פקודות עדכון הטבלאות מופיעות בקובץ Integrate.sql
