...
**תיאור מילולי:** פונקציה מורכבת המיועדת לסנכרון פיננסי, בקרת קלט ולחישוב העלות הכוללת של הזמנות רכש (רכש סיטונאי מספקים עבור מלאי הרשת). הפונקציה מקבלת מזהה הזמנה (`p_order_id`) ואחוז עלות סיטונאית (`p_wholesale_percentage`). בתחילה, הפונקציה מבצעת בדיקת תקינות (Validation) על אחוז הסיטונאות ומוודאת שהוא בטווח חוקי (בין 1 ל-100), ולאחר מכן בודקת באופן ישיר שההזמנה אכן קיימת במערכת. בהמשך, הפונקציה פותחת סמן מפורש ועוברת בלולאה שורה-אחרי-שורה על כל הפריטים והכמויות המשויכים להזמנה זו מתוך טבלת הקשר `CONTAINS`. עבור כל מוצר, הפונקציה מתעלמת מהנחות הצרכן הרגילות (שכן מדובר ברכש מספק), וגוזרת את המחיר הסיטונאי המוזל ישירות ממחיר המחירון הבסיסי של המוצר בטבלת `PRODUCT` בהתאם לאחוז שסופק בקלט. בסיום החישוב המצטבר, הפונקציה מבצעת פקודת עדכון פיזית (DML) על עמודת `Price` בטבלת האם `"ORDER"` ומחזירה את המחיר הסופי שחושב.

**אלמנטים ותכנוני PL/pgSQL הממומשים בפונקציה (עמידה בדרישות המטלה):**
* **סמן מפורש - Explicit Cursor (סעיף a):** הגדרה ופתיחה ידנית של סמן מפורש (`cur_order_items`) המבצע `JOIN` בין טבלת הקשר `CONTAINS` לטבלת המוצרים `PRODUCT` ומאפשר עיבוד נתונים מבוקר של המוצרים בהזמנה.
...
* **רשומות - Records (סעיף g):** שימוש ברשומה דינמית (`r_item` מסוג `RECORD`) לקליטה זמנית של נתוני השורות (מחיר וכמות) הנשלפות מהסמן בכל איטרציה של הלולאה.
* **פקודות DML ועדכון נתונים (סעיף c):** ביצוע פקודת `UPDATE` ישירה על טבלת `"ORDER"` לעדכון מחיר הרכש הסופי המשוקלל בבסיס הנתונים מתוך הפונקציה.
* **הסתעפויות (סעיף d):** שימוש במבני תנאי (`IF THEN`) לביצוע ולידציות קריטיות: בדיקה שאחוז הקלט תקין, בדיקה אם ההזמנה קיימת במערכת, ובדיקה נוספת למניעת חישוב עבור הזמנות "רפאים" ללא מוצרים.
...
### 📝 תוכנית ראשית 1: עדכון פחת והפקת דו"ח מבצעים אקטיביים

תוכנית זו משלבת תהליך תפעולי ושיווקי במקביל ומבצעת את הפעולות הבאות:

1. **החלת הנחת פחת (פרוצדורה):** מאתרת אוטומטית בטבלת `PRODUCT` את כל המוצרים שתוקפם עומד לפוג ב-45 הימים הקרובים, ומורידה את מחיר המחירון שלהם ב-**15%**.
2. **שליפת מבצעים (פונקציה):** קוראת לפונקציה שמחזירה סמן דינמי (`Ref Cursor`) המכיל את כל ההנחות והמבצעים התקפים ליום הנוכחי.
3. **הפקת דו"ח (לולאה):** רצה בלולאה שורה-אחרי-שורה על הסמן, ומדפיסה לחלונית ה-`Messages` דו"ח מפורט הכולל את מזהה המבצע, אחוז ההנחה, שם המוצר ותוקפו.
...
### 📝 תוכנית ראשית 2: קליטת משלוח מספק וסנכרון פיננסי של הזמנה

תוכנית זו מייצרת סגירת מעגל מלאה (End-to-End) של תהליך קבלת סחורה ברשת:

1. **קליטת מלאי וסגירת הזמנה (פרוצדורה):** קוראת לפרוצדורה המעדכנת את סטטוס ההזמנה הנבחרת ל-`COMPLETED` ומזרימה אוטומטית את כמויות הפריטים שהגיעו אל תוך טבלת המלאי של הסניף המזמין (`INVENTORY`).
2. **עדכון עלויות סיטונאיות (פונקציה):** מפעילה את הפונקציה הפיננסית שמחשבת את עלות הרכש של ההזמנה מול הספק על בסיס אחוז עלות קבוע (למשל 65% ממחיר הצרכן), ומעדכנת את עמודת `Price` בטבלת `"ORDER"`.
3. **ניהול חריגות (Exception):** עוטפת את כל התהליך בבלוק הגנה ראשי. אם הפרוצדורה נכשלת (למשל, מנסים לקלוט הזמנה שכבר נסגרה בעבר), התוכנית תופסת את השגיאה, מציגה אותה בצורה נקייה ומבצעת Rollback כדי למנוע נזק לנתונים.
