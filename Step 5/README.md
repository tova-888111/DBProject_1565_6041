...
* **שפת פיתוח:** **Python** – נבחרה בשל הגמישות הרבה שלה ויכולת האינטגרציה המהירה מול מסדי נתונים ומנועי תצוגה.
* **ספריות ממשק משתמש (GUI):**
    * **CustomTkinter (ctk):** ספריית שדרוג מתקדמת מעל Tkinter המספקת רכיבים ויזואליים מודרניים עם תמיכה מובנית בעיצובים כהים/בהירים, פינות מעוגלות, רכיבי חלוניות מודאליות (`CTkToplevel`) ומערכת כפתורים מתקדמת.
    * **Tkinter & TTK (Treeview, Style):** שימוש ברכיבי הטבלאות המתקדמים של TTK להצגת נתונים דינמית, תוך החלת סטייל מותאם אישית (כיתוב מודגש בצבע כחול עמוק, סימון שורות בכחול בהיר וצביעת שורות מותנית בהתאם לסטטוסים).
* **קישוריות לבסיס הנתונים:** **Psycopg2 / db_connection** – לחיבור ישיר, מאובטח ומהיר מול שרת ה-PostgreSQL של הרשת, המאפשר ביצע שאילתות, הרצת פרוצדורות (`CALL`) ופונקציות שרת (`SELECT calc...`).
* **מנגנוני הגנה ויציבות (Data Integrity):** הטמעת בלוקי `try-except` ברמת ה-GUI לתפיסת חריגות שרת (כגון מפתחות כפולים `Duplicate Key` ומגבלות מפתח זר `Foreign Key Constraint`), והצגתן למשתמש כהודעות אזהרה ידידותיות בעברית (`messagebox`).
...
* **טבלאות מעורבות:** טבלת הזמנות (`ORDER`), טבלת תכולת פריטים (`CONTAINS`), טבלת רכבי הפצה (`TRUCK`) טבלת חברות משלוחים (`DELIVERYCOMPANY`) ,  חברת משלוחים איזורים (`DELIVERYCOMPANY_REGIONSERVED`).
* **מה ניתן לעשות במסך זה:**
