...
class RamiLeviApp(ctk.CTk):
    def __init__(self):
        ...
        # הגדרות חלון - רחב ומקצועי כמו בהשראה
        ...
        # -------------------------------------------------------------
        # 1. תפריט צד ימני (Right Sidebar) - כחול-נייבי כהה ועמוק מאוד
        # -------------------------------------------------------------
        ...
        if os.path.exists(self.logo_path):
            # שימוש ב-Pillow לטעינת התמונה בצורה חלקה ונקייה
            ...
            # גיבוי טקסטואלי אם הקובץ לא נמצא זמנית
        ...
        self.sub_logo.pack(pady=(0, 45))

        # מילון לשמירת כפתורי הניווט
        self.buttons = {}
        
        # יצירת כפתורי הניווט בדיוק לפי צילום המסך (עם פונט גדול וברור יותר)
        ...
        # -------------------------------------------------------------
        # 2. האזור המרכזי הדינמי - רקע ווב בהיר, מרווח ואלגנטי
        # -------------------------------------------------------------
        self.main_frame = ctk.CTkFrame(self, fg_color="#F3F4F6", corner_radius=0) # צבע רקע אפרפר-בהיר נעים לעין
    ...
    def create_sidebar_button(self, key, text, command):
        ...
        btn = ctk.CTkButton(
            ...
            hover_color="#111827", anchor="e", height=50, corner_radius=10,
            command=command
        )
    ...
    def set_active_button(self, active_key):
        """אפקט סימון ירוק-ברקת בולט לכפתור הפעיל, כמו בתמונה"""
